...
- `backup_course.py` — local course backup zip (`--course`, optional `--no-sources`, `--output-dir`, `--list`, `--overwrite`); default output `08_App_Data/exports/backups/`; includes `STUDYFORGE_BACKUP_INFO.md`; no cloud upload; restore is manual in v1; GUI **Courses** → **Course backup**
