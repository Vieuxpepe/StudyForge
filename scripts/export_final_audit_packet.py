#!/usr/bin/env python3
"""
Export a final audit packet for manual paste into ChatGPT / strong models.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Export final audit packet (Markdown + JSON)."
    )
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Replace existing final audit packet Markdown and JSON.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Error: {exc}", file=sys.stderr)
        return 1
    except NoIntermediateAuditError as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 1
    except FinalPacketOutputExistsError as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 1
    ...
    print("Final audit packet exported.\n")
    print(f"Course:\n{summary['course']}\n")
    print(f"Source:\n{summary['source_id']} - {summary['title']}\n")
    print(f"Intermediate audit:\n{summary['intermediate_audit_id']}\n")
