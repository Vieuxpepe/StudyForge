#!/usr/bin/env python3
"""
Import a manual final audit (e.g. from ChatGPT) into StudyForge.

Examples:
    python scripts/import_final_audit.py --course ECA1010_Microeconomics --source-id SRC-0001 --file "C:\\path\\final_audit.md"
    python scripts/import_final_audit.py --course ECA1010_Microeconomics --source-id SRC-0001 --text "Final audit content"
"""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Import a manual final audit (versioned storage)."
    )
    ...
    parser.add_argument(
        "--auditor-name",
        default="ChatGPT",
        help='Name of the auditor (default: "ChatGPT").',
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Error: {exc}", file=sys.stderr)
        return 1
    except FinalAuditNotReadyError as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 1
